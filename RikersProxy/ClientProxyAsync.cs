...
{
    public class ClientProxyAsync : IClientProxyAsync
    {
        ...
        */
        ...
        public async Task<ProxyResult> ObtainToken()
        {
            ...
            if (validateOk)
            {
                ...
                using (var client = new ClientApi())
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        return new ProxyResult { Code = new HttpStatusCode(), Message = "Falta Error", Messages = MessageLts };
        ...
        public async Task<ProxyResult> CreateCase(CaseData data)
        {
            List<Message> MessageLts = new List<Message>();
            ...
            if (Elapsed_Time_Token.TotalMinutes > 0)
            {
                ...
                using (var client = new ClientApi())
                {
                    ...
                    try
                    {
                        ...
                        var task = new Task<IRestResponse>(() => client.CreateCase(this.Token.AccessToken, data));
                        task.Start();
                        ...
                        if (response.StatusCode == HttpStatusCode.Created)
                        {
                            ...
                            var msg = $"successfull case created id:{casecreated.CaseNumber} - DateCreate: {casecreated.CommonArea.TransactionDate.ToString("yyyy-MM-dd HH:mm:ss")}";
